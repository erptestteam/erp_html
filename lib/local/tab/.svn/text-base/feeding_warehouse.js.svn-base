...
var entity_edit_model = {
	...
	dispaly_columns : [ {
		...
		width : 100,
		render : function(rowdata, rowindex, value) {
			if (rowdata.storage_mark + '' == '0'){
				if (rowdata.feeding_status_now == rowdata.feeding_status_all) {
					return "<button onClick='finish_storage(\""+rowdata.item_number+"\","+rowdata.id+")'>入库</button>";
				}
				else if(rowdata.feeding_status_now < rowdata.feeding_status_all){
					return "等待";
				}
				else if(rowdata.feeding_status_now > rowdata.feeding_status_all){
					return "错误";
			...
			else if (rowdata.storage_mark + '' == '1'){
				return "结束";
			}
		}
	}, {
...
function finish_storage(item_number,feeding_id){
	var param = {
	...
	show_item_storage(item_number);
}
function show_item_storage(item_number)
{
